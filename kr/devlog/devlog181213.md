...
많은 인용이 증명했듯, 위 글이 정확히 제가 원하던 내용을 담고 있었습니다.

![마우스 경로가 조건에 맞으면 하위 메뉴를 띄워두는 예시](https://static.tumblr.com/9hgswys/vogmj8fh4/screen_shot_2013-03-06_at_1.17.35_am.png)

아마존 메가 메뉴는 아래 로직으로 마우스 이벤트를 억제했습니다.

- 상위 메뉴에 있는 마우스 커서와 하위 메뉴의 좌측 변을 연결해서 삼각형 영역을 만들어 줍니다.
- 커서가 해당 영역 안에 있다면 '하위 메뉴를 향해 가는 중'이라고 판단하고 다른 상위 메뉴를 지나가더라도 노출하고 있는 하위 메뉴를 그대로 보여줍니다.

즉, 위 이미지에서 보이는 것처럼 'Amazon Cloud Drive'에 있는 커서가,  
하위 메뉴를 향해 움직이다 아래에 있는 'Appstore for Android' 항목을 지나가더라도, 삼각형 영역 안을 지나간다면 여전히 하위 메뉴는 'Amazon Cloud Drive'의 내용을 보여주게 됩니다.
...
다시 자료 수집부터 시작했습니다. 구글에서 'JavaScript draw triangle'을 검색해보니 MDN의 **Canvas API 소개 글** [**'Drawing shapes with canvas'**](https://developer.mozilla.org/en-US/docs/Web/API/Canvas_API/Tutorial/Drawing_shapes)이 눈에 먼저 들어왔습니다.

해당 문서 첫 장에 있는 'Drawing a triangle' 문단에서, Canvas를 이용해 삼각형을 그리는 아래 예시를 찾을 수 있었습니다.
